...
bool ndt_mapping::init()
{
 ...
 // vector6 << 1e-6, 1e-6, 1e-6, 1e-8, 1e-8, 1e-6;
  ...
  prior_noise_ = noiseModel::Diagonal::Variances(Vector6_prior);
  odom_noise_ = noiseModel::Diagonal::Variances(Vector6_odom);
...
void ndt_mapping::performLoopClosure()
{
  ...
  if( (has_converged == false )|| (fitness_score > history_fitness_score_))
  {
    if (has_converged == false )
      ROS_WARN("oop cannot closed: has_converged == false");
    else
      ROS_WARN("loop cannot closed:fitness_score > history_fitness_score_  %f: ",fitness_score); 
...
bool ndt_mapping::detectLoopClosure()
{
  ...
  if (pub_history_keyframes_.getNumSubscribers() > 0)
  {
    ...
    msg.header.stamp = current_scan_time;
    msg.header.frame_id = "map";
